...
class GoalsProvider extends ChangeNotifier {
  ...
  // Stability/skip-risk model adapted from Persist V3's weighted heuristic.
  // It is intentionally rule-based, explainable, and presentation-safe.
  int get stabilityScore {
    return (100 - (skipProbability * 100)).round().clamp(0, 100);
  }

  double get skipProbability {
    final openTasks = getTodayTasks().length;
    final completionFactor = (1 - overallCompletionRate).clamp(0.0, 1.0);
    final overdueFactor = (totalOverdueDays / 5).clamp(0.0, 1.0);
    final openTaskFactor = (openTasks / 8).clamp(0.0, 1.0);
    final coldStartFactor = totalCompletedTasks == 0 ? 0.55 : 0.18;
    final streakProtection = (currentStreak / 14).clamp(0.0, 1.0) * 0.20;

    final risk =
        completionFactor * 0.34 +
        overdueFactor * 0.28 +
        openTaskFactor * 0.18 +
        coldStartFactor * 0.12 -
        streakProtection;

    return risk.clamp(0.04, 0.94);
  }
