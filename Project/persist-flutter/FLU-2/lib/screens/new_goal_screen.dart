...
class _NewGoalScreenState extends State<NewGoalScreen> {
  ...
  Future<void> _generateAiPlan() async {
    ...
    try {
      ...
      final safePlan = (plan == null || plan.length < _days)
          ? ai.generateLocalGoalPlan(_nameCtrl.text.trim(), _days, _category)
          : plan;
      if (mounted) {
        setState(() {
          _aiPlan = safePlan;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Generated ${safePlan.length}-day plan. Tap Create Goal to save.')),
        );
      }
    } catch (_) {
      final safePlan = ai.generateLocalGoalPlan(_nameCtrl.text.trim(), _days, _category);
      if (mounted) {
        setState(() {
          _aiPlan = safePlan;
          _aiLoading = false;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Generated ${safePlan.length}-day fallback plan. Tap Create Goal to save.')),
        );
      }
    }
  }
